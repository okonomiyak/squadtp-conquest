...
/復元方式**へ全面置き換え。さらに**最大HPのconfig化**(`maxHealth`、銃Modとのバランス調整用)も
追加。詳細は下記および**README.md**参照。
...
- **最大HPをconfigで調整可能に**(2026-08-07新規実装): 新規`maxHealth`(既定20.0、バニラ標準)。
  ...
  2箇所で呼ぶ: (1)`joinTeam`(ただし実際にチームが変わった時のみ — 同じチームへの
  `/conquest team join`連打でヒールを繰り返せる抜け穴を防ぐガード)、(2)`teleportToSpawns`
  (ラウンド開始時の一括ループ、こちらは無条件で全参加者に適用 — config変更後も次ラウンドで
  確実に反映される)。管理人チームでは常にバニラの20に戻す。`/conquest config set maxHealth <値>`
  にも対応(`CONFIG_KEYS`に登録済み、再起動なしで変更可)
...
1. チームビーコン/拠点スポーンの選択UI化、AED(分隊外蘇生)、拠点のJourneyMapウェイポイント表示、
   ...
   スナップショット復元、最大HPのconfig化の実プレイテスト(上記「未検証」参照、最優先。

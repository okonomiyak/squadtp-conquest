...
- **v3.0リリース**(2026-08-20): ユーザー指定で`mod_version`を`0.2.11`から`3.0`へメジャー
  バンプ(通常の0.2.x系の1つ上げではなく、ユーザーから明示的に「ver3.0としてリリースして」
  と指定されたバージョン番号)。v0.2.11以降の8コミット分(旗ブロック右クリックのメニュー
  無効化・戦場境界スナップショットの撮影タイミング修正・キルフィード追加・ダウン時キル
  二重加算バグ修正・スコア画面のK/D/蘇生数表示・通算成績のチーム統合+K/D個別数値化)を
  まとめてリリース。squadtp側は変更無し(依存は引き続き0.2.7)。
- **通算成績のK/Dを個別数値に分割**(2026-08-20): ユーザーから「K,D,K/D比,蘇生」と指定
  (直前の変更で`12/5`のようにまとめていたK/Dを、キル数・デス数を別々の数値として出す
  よう訂正)。`drawLifetimeRow`のフォーマットを`kills/deaths  (比率)  蘇生`から
  `kills  deaths  (比率)  蘇生`に変更、列ヘッダーも「K/D  (Ratio)  Revives」から
  「K  D  K/D  Revives」に更新。
- **通算成績のチーム分けを廃止し表示項目もK/D/K比率/蘇生数に変更**(2026-08-20): ユーザーから
  「通算成績はK D K/D 蘇生数でチーム分けはいらない」と依頼。`ConquestScoreScreen`の
  「通算成績」ページ(ヘッダーのボタンで切替)が従来チームA/B別の2カラム表示だったのを、
  単一の統一ランキングリストに変更(`renderLifetimeColumn`から`team`引数を削除、
  `sortedTeamLifetime(all, team)`だった振り分けロジックを`sortedLifetime(all)`
  ——`ADMIN`/`SPECTATOR`以外全員を1本のリストにまとめてスコア降順ソート——に置き換え)。
  通算成績は複数ラウンドの合計なので、「今の」所属チームで分けると、ラウンド間でチーム移動
  した場合に不自然になるという判断。各行の表示内容も`Score K/D K/D比率`から
  `K/D K/D比率 蘇生数`に変更(スコア自体は非表示になったが、順位付けの基準としては
  内部的に引き続き総合スコア降順を使用)。列ヘッダーのテキストも合わせて更新。
...
- **ダウン時キル加算が二重計上されるバグを修正**(2026-08-20): ユーザーから
  ...
  squadtp-conquestの`ScoreEvents.onDeath`(実際の死亡経由でキルを加算する側)がどちらも
  優先度未指定=`NORMAL`だったため、2つの別Mod間での実行順序が実質未定義だった。実行順序が
  「squadtp-conquest側が先」になった場合、`onDeath`がまだキャンセルされていないイベントに
  対して先にキルを加算し、その直後にsquadtpがイベントをキャンセルしてダウン状態に変換・
  `PlayerDownedEvent`を発火、それを`onPlayerDowned`が拾って**同じキルをもう一度**加算して
  いた(逆順なら`onDeath`はForgeの既定`receiveCanceled=false`によりスキップされ問題なし
  ——実行順序次第で再現有無が変わるバグだった)。`onDeath`に`@SubscribeEvent(priority =
  EventPriority.LOW)`を指定し、squadtpの`NORMAL`優先度リスナーより確実に後で実行される
  ようにすることで解決(squadtpのキャンセル判定が先に確定してから`onDeath`が呼ばれるので、
  ダウン変換時は常にスキップされるようになった)。
- **画面右上にキルフィードを追加**(2026-08-20): ユーザーから
  「ダウンしてら右上にキルログが流れてほしい ユーザー1→ユーザー2みたいな」と依頼。新パケット
  `KillFeedPacket`(攻撃者名・被害者名・表示秒数)を追加し、キルが記録される瞬間
  (`ScoreEvents.onPlayerDowned`のダウン時クレジット、および`onDeath`側の蘇生無効時/即死
  ダメージ経路、両方から共通ヘルパー`broadcastKillFeed`経由)に全員へブロードキャスト
  (`PacketDistributor.ALL`)。クライアント側は`ConquestClientData`に既存のスポット/ピンと
  同じ「絶対ゲーム内時刻での有効期限つきリスト」パターンで保持し、`killFeedDurationSeconds`
  (既定5秒)経過で自動的に消える(フェードアウトは無し、単純に消える)。新規オーバーレイ
  `KillFeedOverlay`(`ConquestHudOverlay`と同じ`IGuiOverlay`パターン)で画面右上に「攻撃者名
  → 被害者名」を最大5件、新しい順に上から表示。表示は蘇生システムの有効/無効を問わずキルが
  記録される全経路をカバー(ユーザーの依頼は「ダウンしたら」だったが、蘇生無効サーバーでも
  キルフィードが動くよう、実際の死亡経由のキルもあわせて対応)。新パケット追加のため
  `PROTOCOL_VERSION`を19→20にバンプ。
- **旗ブロックの右クリックでメニューを開かないように**(2026-08-20): ユーザーから
  「旗を触ったときメニューを開かなくていい」と依頼。`ConquestFlagBlock.use()`が右クリックで
  `/conquest`のGUIを開くだけの実装だった(Lキーと全く同じ画面を開く冗長な経路)ため、
  メソッドごと削除(未使用になった`ServerPlayer`/`MinecraftServer`/`InteractionHand`/
  `InteractionResult`/`Player`/`BlockHitResult`/`ConquestManager`/`NetworkHandler`の
  importも整理)。旗ブロックはこれで純粋に見た目・当たり判定のみの静的オブジェクトになり、
  GUIを開く手段はLキーのみに一本化された(戦闘中に誤って旗をクリックして意図せずメニューが
  開いてしまう事故も無くなる)。
- **戦場境界の地形スナップショットを境界設定時に撮影するよう変更**(2026-08-20): ユーザーから
  「戦闘区域の保存システムもさその範囲を決めた地点のものに戻るようにして」と依頼(演習場が
  `/conquest range set`実行時にスナップショットを撮る方式に対し、戦場境界は`/conquest start`の
  たびに毎回撮り直していた、という不一致を指摘)。従来方式だと、あるラウンドで地形破壊を直しきれ
  なかった場合、その壊れた状態が次のラウンドの「復元先」として定着してしまい、境界を最初に
  設定した時点の本来の地形へは二度と戻せなくなるという実質的なバグだった。`setBoundary`/
  `setBoundaryCorner`(両角が揃った瞬間)から`captureTerrainSnapshot`を呼ぶよう変更し(演習場の
  `setRange`/`setRangeCorner`と全く同じパターン)、`start()`側は`terrainSnapshot == null`の
  時だけ撮影するフォールバックに縮小(グローバル境界を使わずセクター戦闘エリアのみで運用する
  ブレイクスルー向け、およびサーバー再起動でスナップショットを失った場合の自己修復用——
  どちらも演習場の`tickRange`にある同種のフォールバックと同じ理屈)。
- **GUIのチームA/B参加ボタンを削除**(2026-08-19): ユーザーから「GUIのチーム参加ボタンをなくして」
  と依頼。`ConquestScreen`の状況タブにあった「チームAに参加」「チームBに参加」の2ボタン
  (`/conquest team join a|b`を送信するだけの単純なショートカット)を削除。直前に追加した
  `teamJoinRequiresOp`/`lockTeamChangeDuringRound`はコマンド側でしか判定していないため、
  ボタンを残したままだとGUIから押した場合の可否表示(有効/無効化・エラー文言)をクライアント側
  にも複製する必要が出てしまう——チーム参加は`/conquest team join`のチャットコマンドのみに
  一本化し、判定ロジックの重複を避けた。レイアウト上ボタン用に確保していた縦幅も削減
  (`cursor = teamY + 36` → `teamY + 16`)。未使用になった言語キー`conquest.gui.team_a`/
  `team_b`も削除。
- **v0.2.11リリース**(2026-08-18): v0.2.10以降の8コミット分(JDKパスのハードコード除去・
  ドキュメントからのユーザー名除去・チーム参加時のロードアウト装備追随・待機チーム挙動の
  撤回/修正2件・ダウン時キル加算・`teamJoinRequiresOp`・squadtpのダウン中ポーズメニュー
  修正)をまとめてリリース。squadtp側は`PlayerDownedEvent`公開APIとダウン中ポーズメニュー
  修正を含む0.2.7に追従。
- **ダウン中の一時停止メニューを禁止**(2026-08-17): ユーザーから
  「設定開いてるときにダウンすると死ななくなって30秒間なんもできない」と報告(具体的には
  「Gキー(ギブアップ)が押せない」)。原因はsquadtp本体側: ダウン中に開ける画面はチャット/
  一時停止/デス画面のみと制限していたが、シングルプレイ/LAN公開でホストしている場合、
  一時停止メニューを開くと統合サーバー自体が完全に止まり(ワールドtickが停止)、
  ギブアップキー長押しでコマンドは送信されるものの`ReviveSystem`側での実際の処理は
  ポーズ解除まで一切走らない、という組み合わせで「死ぬこともギブアップすることもできず
  固まる」不具合が発生していた。squadtp本体の`client.ClientEvents.onScreenOpening`
  (`ScreenEvent.Opening`購読)から`PauseScreen`を許可リストから除外するだけで対処
  (専用サーバーでの通常プレイには影響無し、`isPaused()`が真になるのは統合サーバーの時だけ
  のため)。squadtp-conquest側の変更は無し、squadtp本体のみのコミット。
...
- **待機チーム参加時のテレポートを撤回**(2026-08-16): ユーザーから「watingのときのtpはなし」
  ...
  1つ前の変更で追加した`/conquest waiting spawn set|remove|list`・`waitingSpawnDim`/
  `waitingSpawnPos`フィールド・`joinTeam`の`teleportToWaiting`呼び出し・NBT永続化・
  対応する言語キーを丸ごと削除。`waiting`参加時はインベントリクリアのみ行い、参加した位置に
  留まる(この機能追加自体はv0.2.10で一度リリース済みだったが、実プレイ検証前に方針転換された
  ため撤回)。
- **試合中のA/B参加でもロードアウトを装備**(2026-08-16): ユーザーから
  「戦闘中と演習中でのみロードアウトが反映されるようにして」と依頼。直前に追加した
  「ラウンド進行中に`/conquest team join a|b`したらスポーンにtp」(`joinTeam`の
  `team.isCombatant() && state == RoundState.IN_PROGRESS`分岐)が、テレポートだけで
  `ClassLoadoutCompat.equip`を呼んでおらず、既存の「ラウンド開始」「演習場」の2箇所とだけ
  装備タイミングが揃っていなかった抜けを指摘されたもの。同分岐に`ClassLoadoutCompat.equip
  (player)`を1行追加するだけで解消(死亡を経由しない直接テレポートなのでclassloadout自身の
  自動装備フックでは拾えない、という既存の理屈がそのまま当てはまる)。これで
  ロードアウトが自動装備されるのは戦闘中(A/Bで`IN_PROGRESS`中の直接テレポート全て)と
  演習中(`range`)の2ケースに揃った。
- **v0.2.10リリース**(2026-08-16): v0.2.9以降の3コミット分(待機チーム参加時のテレポート・
  試合中にA/B参加でスポーンにtp・`lockTeamChangeDuringRound`config)をまとめてリリース。
  squadtp側は変更無し(依存は引き続き0.2.6)。
...
- **試合中にA/Bへ参加したらスポーンにtp**(2026-08-16): ユーザーから「試合中にa,bのチームに
  ...
  `onRespawn`頼み)、ラウンド進行中に`/conquest team join a|b`で新規参加・チーム移動した
  プレイヤーはその場に取り残されていた。`joinTeam`の共通処理末尾に
  `team.isCombatant() && state == RoundState.IN_PROGRESS`の分岐を追加し、既存の
  `teleportToRoleSpawn`(ラウンド開始・チームスポーン選択で使っているのと同じメソッド)を
  呼ぶだけで対応。待機中(`state != IN_PROGRESS`)の参加は従来通りラウンド開始の一括テレポートに
  任せるため対象外。

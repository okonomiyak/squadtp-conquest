...
- **右Altキー**(Tabキーはバニラのまま変更していない): BF風スコアボード。パネルはグレー半透明で
  ...
  チケットバー・拠点アイコン列(拠点ごと)・セクター差分・デス数・自分のK/D/蘇生数
  (2026-08-20変更、アシスト数の代わりに表示。アシストのスコア計算自体は変更無し)、
  ...
  - ヘッダーのボタンで「今ラウンド」⇔「**通算成績**」を切り替え可能。通算成績は
    セッション中の全ラウンド合計で、**チーム分けはしない**単一の順位付きリスト
    (2026-08-20変更。通算成績は複数ラウンドをまたぐため、集計対象プレイヤーの「現在の」
    チームで分けると、ラウンド間でチームを移動した場合に見え方が不自然になるため統一)。
    各行はキル数・デス数・K/D比率・**蘇生数**を個別の数値として表示
    (2026-08-20さらに変更、K/Dを`12/5`のようにまとめず分けて表示。スコアは非表示のまま、
    順位自体は内部的に総合スコア降順)
...
- **画面右上にキルフィード**(2026-08-20追加): キルが記録される(蘇生システム有効時はダウンさせた
  瞬間、無効時や即死ダメージ時は実際の死亡時)たびに「攻撃者名 → 被害者名」を右上に表示。全員に
  ブロードキャストされる(チーム不問)。表示時間は`killFeedDurationSeconds`(既定5秒)。フェード
  アウトはせず、時間経過で単純に消える(スポット/ピンと同じ挙動)
...
- **同期**: `ConquestSyncPacket`(全拠点の状況リスト・チケット・ラウンド状態など)と
  `ConquestScoreboardPacket`(全参加者のK/D/蘇生数)は毎秒全員に配信。他に単発イベント用の
  S2Cパケット([スポット](#索敵マーキングスポット)・[ピン](#ピン)・[キルフィード](#hud常時表示))
  もある
...
- `killFeedDurationSeconds`(既定5) — 画面右上のキルフィード表示が消えるまでの秒数

...
同じディメンションのはずなので実運用上は問題にならない)。`/conquest start`のたびにその範囲全体を
まるごとスナップショット(バニラの構造物ブロックと同じ`StructureTemplate`の仕組みを利用)し、
ラウンド終了(`endRound`、勝敗タイトル表示・集合地点へのテレポートと同じタイミング)でその全体を
貼り戻す。爆発1つ1つの破壊を個別に記録する方式ではないため、爆発以外の要因で範囲内の地形が変わっても
まとめて元に戻る(逆に**範囲の外**で起きた破壊は対象外)。`/conquest stop`では復元しない(停止直後に
被害状況を確認したい場合を想定、`endRound`とは別経路)。復元したくなったら`/conquest boundary restore`
で手動復元できる。**このコマンドはスナップショットを消費せず、何度でも呼び直せる**(壊す→復元→
また壊す→また復元、を同じラウンド内で好きなだけ繰り返せる。次の`/conquest start`で新しい
スナップショットに置き換わるまで、同じ内容がずっと保持される——コマンド名は`boundary`だが対象が
セクター戦闘エリア由来のフォールバック範囲でも同じコマンドで復元できる)。**実行した瞬間には
まだ復元されない**——即座に全員へチャット警告が流れ、`terrainRestoreDelaySeconds`秒(既定5)後に
実際に貼り戻される(2026-08-15追加。範囲内にいるプレイヤーが巻き込まれる前に離脱する猶予を
作るため)。ラウンド終了時の自動復元(`endRound`)や演習場の定期リセットはこの遅延の対象外で、
従来通り即座に実行される。
境界もセクター戦闘エリアも無ければ地形の自動復元は一切働かない(クレーター自体は生成される)。
対象範囲が非常に広い場合、開始時・終了時にその範囲分の一括処理コストがかかる点に注意(上限キャップは
現状無し)。スナップショットはラウンドスコープの一時状態でありNBTに永続化されない

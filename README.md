...
| `/conquest team join <a\|b\|admin\|range\|spectator> <プレイヤー\|セレクター>` | 指定したプレイヤーをそのチームに参加させる(自分以外も指定可)。`@a`・`@e[team=...]`等の複数対象セレクターにも対応(該当者全員をまとめて参加させる)。対象にも参加したことがアクションバーで通知される | OP |

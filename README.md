...
境界が未設定なら地形の自動復元は一切働かない(クレーター自体は生成される)。境界が非常に広い場合、
...
6. リスポーンするたびに、そのプレイヤーの**自チームの**チケットを`ticketCostPerRespawn`だけ消費
   ...
   タイミングでのみ発生)。[チームリスポーンビーコン](#チームリスポーンビーコン)や
   (`spawnAtOwnedPointsEnabled`有効時の)保有拠点は、squadtpの死亡後リスポーン選択画面で
   プレイヤー自身が選べる選択肢として提示される(選ばなければ`/conquest spawn set`のチーム固定
   スポーン、未設定ならワールドスポーン)

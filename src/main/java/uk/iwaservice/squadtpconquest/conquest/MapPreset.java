...
public final class MapPreset {
    ...
    static MapPreset load(CompoundTag tag) {
        ...
        if (tag.contains("SpawnA2Dim")) {
            spawnA2Dim = ResourceKey.create(Registries.DIMENSION, new ResourceLocation(tag.getString("SpawnA2Dim")));
            spawnA2Pos = NbtUtils.readBlockPos(tag.getCompound("SpawnA2Pos"));
        }
        ResourceKey<Level> spawnB2Dim = null;
        BlockPos spawnB2Pos = null;
        if (tag.contains("SpawnB2Dim")) {
            spawnB2Dim = ResourceKey.create(Registries.DIMENSION, new ResourceLocation(tag.getString("SpawnB2Dim")));
            spawnB2Pos = NbtUtils.readBlockPos(tag.getCompound("SpawnB2Pos"));
        }
